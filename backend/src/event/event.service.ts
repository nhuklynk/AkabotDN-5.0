...
export class EventService {
  ...
  async update(
    ...
    if (
      updateEventDto.tag_ids !== undefined &&
      updateEventDto.tag_ids !== null
    ) {
    ...
    if (
      updateEventDto.category_ids !== undefined &&
      updateEventDto.category_ids !== null
    ) {
  ...
  async findAll(
    ...
    if (start_date && end_date) {
      queryBuilder.andWhere(
        'event.start_time BETWEEN :start_date AND :end_date',
        {
          start_date,
          end_date,
        },
    ...
    if (countdown_enabled !== undefined) {
      queryBuilder.andWhere('event.countdown_enabled = :countdown_enabled', {
        countdown_enabled,
      });
    }

    queryBuilder.orderBy('event.start_time', 'DESC').skip(skip).take(limit);
  ...
  async findByTagId(
    tagId: string,
    queryDto?: TagEventQueryDto,
  ): Promise<PaginatedData<EventResponseDto>> {
    const { page = 1, limit = 10, start_date, end_date } = queryDto || {};
  ...
  async findByCategoryId(
    categoryId: string,
    queryDto?: CategoryEventQueryDto,
  ): Promise<PaginatedData<EventResponseDto>> {
    const { page = 1, limit = 10, start_date, end_date } = queryDto || {};
  ...
  async getEventStatistics(): Promise<EventStatisticsDto> {
    ...
    const avgEventsPerMonth =
      currentMonth > 0 ? eventsThisYear / currentMonth : 0;
  ...
  async getMonthlyEventStatistics(
    ...
    const monthlyStats = await this.eventRepository
      ...
      .where('EXTRACT(YEAR FROM event.created_at) = :year', {
        year: currentYear,
      })
  ...
  async getEventLocationStatistics(
    ...
    return locationStats.map((stat) => ({
      ...
      percentage:
        totalEvents > 0
          ? Math.round((parseInt(stat.event_count) / totalEvents) * 10000) / 100
          : 0,
    }));
  }
  ...
  async getEventCategoryStatistics(
    ...
    return categoryStats.map((stat) => {
      ...
      const percentage =
        totalEvents > 0
          ? Math.round((eventCount / totalEvents) * 10000) / 100
          : 0;
   ...
   */
  private async mapEventsToResponseDtos(
    events: any[],
  ): Promise<EventResponseDto[]> {
